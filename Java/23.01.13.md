...
Runnable implements 하기에 Thread의 Runnable 파리미터 생성자를 통해 Thread의 메서드의 Start()를 통해 A class의 run()을 thread로 등록할 수 있다.
...
## 오늘 할 일
1. ileWriterTest01.class bw.write()대신 newLine()으로 바꿔보기 [ X ]
2. UserVO.class 만들어서 ReadObjcetFile.class, WriteObjcetFile.class 실행 [ X ]
